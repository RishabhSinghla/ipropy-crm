...
async function main() {
  ...
  const changed = sh('git', ['status', '--porcelain']);
  if (!changed.trim()) {
    // A "no change needed" verdict is an answer, not a wait-state: post the
    // model's own reasoning and close the issue, so the CRM's poller marks
    // the report closed and notifies the reporter. Left open with no PR, the
    // report would sit on "the AI is still understanding" forever.
    const summary = String(prBody || '').trim().slice(0, 2000);
    await gh('POST', `/repos/${REPO}/issues/${ISSUE}/comments`, {
      body: [
        'The AI engineer investigated and concluded that no code change is needed.',
        summary ? `\n${summary}\n` : '',
        'Closing this ticket. If the verdict is wrong, comment `ai: go` with more detail and it will take another look.',
      ].filter(Boolean).join('\n'),
    });
    await gh('PATCH', `/repos/${REPO}/issues/${ISSUE}`, { state: 'closed' }).catch(() => {});
    process.exit(0);
  }
