...
`CLAUDE.md` holds what is true everywhere. The detail for one area lives in its own
file, so this one stays short enough to read in a sitting. **Read the matching file
before you work in that area** — each carries traps that have already cost a day.
...
## The Sovereign Financial design, and where it lives

**26 September 2026, the owner, with three prototype files** (a screenshot, a
Tailwind `code.html`, and a `DESIGN.md` of tokens): *"kindly redesign all CRM
according to given three prototypes … reset all key fields and key names as per
given files … padding, colour code, font size, font name, hover style, box
style, chip style, card style … and all functionality should work very smooth."*

**It is a token swap, not a repaint of every screen, and that is the only way
it could be done at all.** Colour, type and shape already resolve through
`styles.css` variables → `tailwind.config.js` tokens → the `.card` / `.btn-*` /
`.input` / `.badge` component classes every module is built from. Changing that
layer moved every screen at once; a per-screen pass would have been hundreds of
files and would have drifted the first time somebody added a panel.

* **The palette.** `--brand-*` is the design's plum (`#701a75` at 600, white on
  it at 10:1), and three voices the old palette had no names for arrived as
  their own tokens: `--accent` (indigo, operational states and entity links),
  `--positive-soft` / `--negative-soft` and their `-on-` pairs. The canvas is
  `#f5f6fa` with `#f2f3ff` as the recessed tone under table headers and field
  tiles.
* **Dark mode needed its own correction and the scan is what found it.** Plum's
  mid-steps are not indigo's: `dark:text-brand-400` landed at **3.3:1** on a
  panel, so every brand-coloured link in dark mode failed — caught by the
  contrast scan on `/settings`, not by anything else. Only the steps used as
  *text* are lifted in `.dark` (300, 400); 500 and 600 stay, because they are
  fills and white on brand-600 must stay at 10:1.
* **The fonts are self-hosted** (`@fontsource-variable/space-grotesk`,
  `@fontsource-variable/geist`), imported at the top of `styles.css`. Not
  Google's CDN: `securityPolicy` in `app.ts` says `style-src 'self'` and names
  no font host, so a `<link>` to fonts.googleapis.com is refused and the CRM
  silently falls back to the system face — the Team-map tile trap in a new
  costume. Space Grotesk carries names, figures and headings (`h1..h3` and
  `.font-display`); Geist carries everything else.
* **`.key-label` / `.key-value` / `.key-tile`** are the design's structural
  flag, the fact under it, and the recessed square it sits in. One class each,
  so a row of keys reads as a ledger rather than as five sentences.
* **The header strip is columns now, not a sentence.** It read
  `Mobile: … Email: … Budget: …` on one line, where label and fact carried the
  same weight; it stacks each pair with a hairline between them.
  `data-testid="header-fields"` and the count-what-fits rule are unchanged, so
  `e2e/splitViewHeader.spec.ts` still measures the same element.
* **The queue card leads with the name and one chip.** The first of the
  module's own subtitle fields becomes a chip beside the name; the rest stay on
  the line under it. Which field that is remains
  `subtitleFields` — Admin → Split View, then the module's flags — so **no
  screen names a field**, and a module that flags nothing shows no chip.

**And the test that was guarding the palette had stopped guarding it.**
`tests/color.test.ts` held the hexes as constants, so when every value in
`styles.css` moved, all six assertions passed against colours the CRM no longer
used. It reads the stylesheet now and derives the surfaces from it too — the
