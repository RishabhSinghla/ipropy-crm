...
// ---------------------------------------------------------------------------
// Linked WhatsApp phones
//
// A rep links their own number here and the CRM sends through it. The pairing
// code is produced by the bridge and shown on this screen, so the rep scans
// something in the CRM with the phone in their hand and never touches the
// machine the bridge runs on.
// ---------------------------------------------------------------------------

outreachRouter.get('/whatsapp-links', asyncHandler(async (req, res) => {
  const user = getUser(req);
  const linked = await import('../../integrations/whatsapp/linkedDevice.js');

  // A rep sees their own; an admin sees the team's, because "why has nothing
  // gone out today" is a question only answerable by looking at all of them.
  const links = user.isAdmin
    ? await linked.listLinks()
    : [await linked.linkForUser(user.id)].filter(Boolean);

  res.json({
    enabled: linked.isLinkedSendingEnabled(),
    sendingHours: { from: linked.SEND_FROM_HOUR, until: linked.SEND_UNTIL_HOUR },
    links,
  });
}));

outreachRouter.post('/whatsapp-links', asyncHandler(async (req, res) => {
  const user = getUser(req);
  await assertCapability(user, 'whatsapp.send');
  const input = z.object({
    label: z.string().max(120).optional(),
    takesUnassigned: z.boolean().optional(),
  }).parse(req.body ?? {});

  const { createLink } = await import('../../integrations/whatsapp/linkedDevice.js');
  // Only an admin decides which number carries the messages nobody owns; that
  // number ends up talking to leads belonging to the whole team.
  res.status(201).json(await createLink({
    userId: user.id,
    label: input.label ?? null,
    takesUnassigned: user.isAdmin ? input.takesUnassigned : false,
  }));
}));

outreachRouter.patch('/whatsapp-links/:id', asyncHandler(async (req, res) => {
  const user = getUser(req);
  const input = z.object({
    takesUnassigned: z.boolean().optional(),
  ...
  const linked = await import('../../integrations/whatsapp/linkedDevice.js');
  const link = await linked.getLink(req.params.id);
  if (!link) throw new NotFoundError('That WhatsApp link no longer exists');
  if (link.userId !== user.id && !user.isAdmin) {
    throw new ForbiddenError('That is not your WhatsApp link');
  }
  // Both of these change how much a number sends, which is what decides whether
  // it survives, so neither is a rep's call to make about their own phone.
  if (!user.isAdmin) throw new ForbiddenError('Only an administrator can change sending limits');

  if (input.takesUnassigned !== undefined) await linked.setTakesUnassigned(link.id, input.takesUnassigned);
  if (input.dailyCap !== undefined) await linked.setDailyCap(link.id, input.dailyCap);
  res.json(await linked.getLink(link.id));
}));

outreachRouter.delete('/whatsapp-links/:id', asyncHandler(async (req, res) => {
  const user = getUser(req);
  const linked = await import('../../integrations/whatsapp/linkedDevice.js');
  const link = await linked.getLink(req.params.id);
  if (!link) throw new NotFoundError('That WhatsApp link no longer exists');
  ...
  await linked.removeLink(link.id);
  res.json({ ok: true });
}));
