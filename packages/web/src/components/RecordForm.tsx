...
function ComparablesHint({
  ...
  const area = typeof values.area === 'number' ? values.area : undefined;
  const relevant = module === 'properties' && field === 'base_price' && Boolean(locality) && bedrooms != null;

  const { data } = useQuery({
    queryKey: ['comparables', locality, bedrooms ?? null, area ?? null],
    queryFn: () => api.comparables({ locality, bedrooms: bedrooms as number, area, excludeRecordId: recordId }),
    enabled: relevant,
