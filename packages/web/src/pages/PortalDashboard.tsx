        ...
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{user?.fullName} · {user?.channelPartnerId ? 'Linked' : 'Unlinked'}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
...
function PortalRecentLeads(): JSX.Element {
  ...
  if (!rows.length) return <p className="text-sm text-slate-500">No leads submitted yet.</p>;
      ...
      {rows.map((r) => (
        <li key={r.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
          <div className="min-w-0">
            ...
            <p className="text-2xs text-slate-500">{String(r.values?.status ?? 'New')}</p>
          </div>
